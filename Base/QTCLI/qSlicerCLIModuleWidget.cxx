...
void qSlicerCLIModuleWidgetPrivate::setupUi(qSlicerWidget* widget)
{
  ...
  this->AutoRunWhenParameterChanged =
    new QAction(q->tr("AutoRun on parameter modified"), autoRunMenu);
  ...
  this->AutoRunWhenInputModified =
    new QAction(q->tr("AutoRun on input modified"), autoRunMenu);
  ...
  vtkMRMLNode* commandLineModuleNode)
{
  ...
  // Populating the UI can set some default values (if there is only 1
  // available node in combobox, then it is chosen automatically), update the
  // CLI to take it into account
  //d->updateCommandLineModuleNodeFromUi(d->CommandLineModuleNode);
}
