...
void qSlicerAppMainWindowPrivate::setupRecentlyLoadedMenu(const QList<qSlicerIO::IOProperties>& fileProperties)
{
  ...
  QAction * clearAction = this->RecentlyLoadedMenu->addAction(
    "Clear Menu", q, SLOT(onFileRecentLoadedActionTriggered()));
...
//-----------------------------------------------------------------------------
void qSlicerAppMainWindow::on_WindowErrorLogAction_triggered(bool show)
{
  Q_D(qSlicerAppMainWindow);
  if (show)
    {
...
//-----------------------------------------------------------------------------
void qSlicerAppMainWindow::on_WindowPythonInteractorAction_triggered(bool show)
{
#ifdef Slicer_USE_PYTHONQT
...
bool qSlicerAppMainWindow::eventFilter(QObject* object, QEvent* event)
{
  ...
  if (showEvent || hideEvent)
    ...
    if (object == this->errorLogWidget())
      {
      d->WindowErrorLogAction->setChecked(showEvent);
      }
#ifdef Slicer_USE_PYTHONQT
    else if (object == this->pythonConsole())
      {
      d->WindowPythonInteractorAction->setChecked(showEvent);
      }
#endif
    }
